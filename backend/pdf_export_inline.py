...
async def export_customer_ledger_pdf(customer_id: str):
    """Export customer ledger to PDF - Each item as separate row"""
    ...
    # Prepare transaction rows with items inline
            ...
            if data.get('items'):
                for idx, item in enumerate(data.get('items', [])):
                    ...
                    row = [
                        ...
                        f"₹{tax_amt:.0f}",
                        f"₹{invoice_total:,.0f}" if idx == 0 else '',
                        '',
                        f"₹{running_balance:,.0f}" if idx == 0 else ''
                    ]
                    txn_data.append(row)
            else:
                txn_data.append([
    ...
    # Create table with smaller font and column widths
